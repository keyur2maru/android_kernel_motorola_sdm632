...
/*
 * Upstream tears a never-published superblock down with destroy_unused_super(),
 * which skips the RCU delay and frees the preallocated shrinker.  4.9's
 * sget_userns() open-codes the same thing as up_write() + destroy_super(), and
 * 4.9 has no shrinker preallocation; wrap that so sget_fc() reads as upstream.
 */
