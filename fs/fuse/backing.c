...
/* 4.9 has no path_put_init(); provide it locally. */
...
int fuse_create_open_initialize(
		...
		struct file *file, unsigned int flags, umode_t mode, int *opened)
{
		...
		struct file *file, unsigned int flags, umode_t mode, int *opened)
{
	...
	err = finish_open(file, entry, fuse_open_file_backing, opened);
				 ...
				 unsigned int flags)
{
	...
	if (backing_file_out)
		...
		/*
		 * 4.9 has no generic_copy_file_range(); it is a thin wrapper
		 * around do_splice_direct().
		 */
		...
		struct kiocb *iocb, struct iov_iter *to)
{
	...
	if (is_sync_kiocb(iocb)) {
		...
		ret = call_read_iter(ff->backing_file, &aio_req->iocb, to);
		fuse_bpf_aio_put(aio_req);
		if (ret != -EIOCBQUEUED)
			fuse_bpf_aio_cleanup_handler(aio_req);
	}
		...
		struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (is_sync_kiocb(iocb)) {
		...
		aio_req->iocb.ki_complete = fuse_bpf_aio_rw_complete;
		refcount_set(&aio_req->ref, 2);
		ret = call_write_iter(ff->backing_file, &aio_req->iocb, from);
		fuse_bpf_aio_put(aio_req);
		if (ret != -EIOCBQUEUED)
			fuse_bpf_aio_cleanup_handler(aio_req);
	}
