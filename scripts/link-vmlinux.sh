...
vmlinux_link()
{
	...
	if [ "${SRCARCH}" != "um" ]; then
		...
		if [[ -n "${CONFIG_THIN_ARCHIVES}" && -z "${CONFIG_LTO_CLANG}" ]]; then
			objects="--whole-archive built-in.o ${1} ${3}"
		else
			objects="${KBUILD_VMLINUX_INIT}			\
				--start-group				\
				${KBUILD_VMLINUX_MAIN}			\
		...
		if [ -n "${CONFIG_THIN_ARCHIVES}" ]; then
			objects="-Wl,--whole-archive built-in.o ${1} ${3}"
		else
			objects="${KBUILD_VMLINUX_INIT}			\
				-Wl,--start-group			\
				${KBUILD_VMLINUX_MAIN}			\
...
# Generate BTF type info and pack it into a relocatable object. It has to
# be produced before kallsyms and fed into every vmlinux_link pass: the
# extra SHF_ALLOC .BTF section shifts symbol addresses, so leaving it out
# of the kallsyms passes would trip the System.map consistency check.
btf_vmlinux_bin_o=""
...
if [ -n "${CONFIG_KALLSYMS}" ]; then
	...
	# step 2
	vmlinux_link .tmp_kallsyms1.o .tmp_vmlinux2 "${btf_vmlinux_bin_o}"
	...
	if [ -n "${KALLSYMS_EXTRA_PASS}" ]; then
		...
		vmlinux_link .tmp_kallsyms2.o .tmp_vmlinux3 "${btf_vmlinux_bin_o}"
...
info LD vmlinux
vmlinux_link "${kallsymso}" vmlinux "${btf_vmlinux_bin_o}"
