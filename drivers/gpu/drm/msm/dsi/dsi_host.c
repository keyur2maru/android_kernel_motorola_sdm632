			...
			struct msm_dsi_phy_shared_timings *phy_shared_timings)
{
	...
	if (!(flags & MIPI_DSI_CLOCK_NON_CONTINUOUS))
		 ...
		 *
		 * Do NOT force the clock lane HS here: on this 14nm PHY a forced
		 * HS clock blocks the data lanes from entering LP escape, so the
		 * LP init commands stall (device #225: forcing HS at this point
		 * made the first init command time out).  The clock lane is
		 * force-requested HS later, at video-engine start
		 * (dsi_op_mode_config), once the LP init sequence has completed.
		 */
...
static int dsi_cmd_dma_tx(struct msm_dsi_host *msm_host, int len)
{
	...
	if (triggered) {
		...
		if (ret == 0) {
			...
			/* Software walk of the hung iova: print the raw
			 * descriptor at each level so the leaf attributes
			 * can be compared against a working mapping.
			 */
			{
				struct msm_drm_private *p3 =
					msm_host->dev->dev_private;
				struct iommu_domain *dom = msm_smmu_get_domain(
					p3->kms->aspace->mmu);
				extern u64 arm_smmu_debug_get_ttbr(
					struct iommu_domain *domain);
				u64 ttbr = arm_smmu_debug_get_ttbr(dom);
				u64 *tbl = phys_to_virt(ttbr & GENMASK_ULL(47, 12));
				int lvl;
				u64 desc = 0;
				u32 va = dma_base;

				for (lvl = 1; lvl <= 3 && tbl; lvl++) {
					int shift = (3 - lvl) * 9 + 12;
					int idx = (va >> shift) & 0x1ff;

					desc = tbl[idx];
					pr_err("%s: pte l%d[%d]=0x%016llx\n",
					       __func__, lvl, idx, desc);
					if (!(desc & 1) || (lvl < 3 && !(desc & 2)))
						break;
					tbl = phys_to_virt(desc & GENMASK_ULL(47, 12));
				}
			}

			/* One-shot probe: retrigger the DMA against an
			 * unmapped iova.  A context fault proves the fetch
			 * issues and the walk runs; silence means the fetch
			 * never leaves the controller.
			 */
			{
				static bool refetched;

				if (!refetched) {
					u32 fsr;
					void __iomem *cbp;
					extern void __iomem *
					arm_smmu_debug_get_cb_base(
						struct iommu_domain *domain);
					...
					msleep(50);
					{
						struct msm_drm_private *p4 =
							msm_host->dev->dev_private;

						cbp = arm_smmu_debug_get_cb_base(
							msm_smmu_get_domain(
								p4->kms->aspace->mmu));
					}
					fsr = cbp ? readl_relaxed(cbp + 0x58) : 0;
					pr_err("%s: unmapped refetch fsr=0x%08x far=0x%08x intr=0x%08x status0=0x%08x\n",
					       __func__, fsr,
					       cbp ? readl_relaxed(cbp + 0x60) : 0,
					       dsi_read(msm_host, REG_DSI_INTR_CTRL),
					       dsi_read(msm_host, REG_DSI_STATUS0));
					if (cbp && fsr)
						writel_relaxed(fsr, cbp + 0x58);
					dsi_write(msm_host, REG_DSI_DMA_BASE,
						  dma_base);
					wmb();
				}
				...
				const struct mipi_dsi_msg *msg)
{
	...
	/* Commands are sent with the video engine transmitting (the command
	 * DMA fetch is only serviced when the transmit scheduler provides
	 * BLLP slots on this controller), so the clock-lane HS force stays
	 * latched across the transfer exactly as the downstream host runs
	 * it; releasing it mid-video kills the stream and with it the
	 * command scheduler.
	 */
